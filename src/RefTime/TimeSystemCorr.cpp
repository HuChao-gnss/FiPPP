...
namespace gpstk
{
   ...
   // changed by ragnwang from -A0-A1*dt to A0+A1*dt 2023-04
   // not gps, but all other system
   ...
   Correction(const CommonTime& ct) const
   {
      ...
      switch(type)
      {
         case GPUT:
            ...
            if(fromTS == TimeSystem::GPS)             // GPS => UTC  // changed by ragnwang -A0-A1*dt
               corr = -A0-A1*dt;
            else                                      // UTC => GPS
               corr = A0+A1*dt;
         ...
         case GAUT:
            ...
            if(fromTS == TimeSystem::GAL)             // GAL => UTC  // changed by ragnwang -A0-A1*dt
               corr = A0+A1*dt;
            else                                      // UTC => GAL
               corr = -A0-A1*dt;
         ...
         case GLUT:
            ...
            if(fromTS == TimeSystem::GLO)             // GLO => UTC  // changed by ragnwang -A0-A1*dt
               corr = A0;
            else                                      // UTC => GLO
               corr = -A0;
         ...
         case GPGA:
            ...
            if(fromTS == TimeSystem::GPS)             // GPS => GAL  // changed by ragnwang -A0-A1*dt
               corr = A0+A1*dt;
            else                                      // GAL => GPS
               corr = -A0-A1*dt;
         ...
         case GLGP:
            ...
            if(fromTS == TimeSystem::GLO)             // GLO => GPS  // changed by ragnwang -A0-A1*dt
               corr = A0;
            else                                      // GPS => GLO
               corr = -A0;
         ...
         case QZGP:
            ...
            if(fromTS == TimeSystem::QZS)             // QZS => GPS  // changed by ragnwang -A0-A1*dt
               corr = 0.0;    // TD?
            else                                      // GPS => QZS
               corr = 0.0;    // TD?
         ...
         case QZUT:
            ...
            if(fromTS == TimeSystem::QZS)             // QZS => UTC  // changed by ragnwang -A0-A1*dt
               corr = A0+A1*dt;
            else                                      // UTC => QZS
               corr =  -A0-A1*dt;
         ...
         case BDUT:
            ...
            if(fromTS == TimeSystem::BDT)             // BDT => UTC  // changed by ragnwang -A0-A1*dt
               corr = A0+A1*dt;
            else                                      // UTC => BDT
               corr =  -A0-A1*dt;
         ...
         case BDGP:
            ...
            if(fromTS == TimeSystem::BDT)             // BDT => GPS  // changed by ragnwang -A0-A1*dt
               corr = A0;
            else                                      // GPS => BDT
               corr = -A0;
         ...
         case IRUT:
            ...
            if(fromTS == TimeSystem::IRN)             // GPS => UTC // changed by ragnwang -A0-A1*dt
               corr = A0+A1*dt;
            else                                      // UTC => GPS
               corr = -A0-A1*dt;
