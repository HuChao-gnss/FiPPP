...
public:
   ...
   void readASCIIheader(std::string filename) noexcept(false);
   ...
   int readASCIIdata(std::vector<std::string>& filenames) noexcept(false);
   ...
   ///        from the header value.
   int readASCIIdata(std::string filename) noexcept(false);

   /// Write the header (ASCII) to an output stream
   /// @throw if any stream error occurs
   /// @return 0 success,
   ///        -4 header has not yet been read.
   int writeASCIIheader(std::ostream& os) noexcept(false);
   ...
   /// @return 0 success,
   ///        -4 header has not yet been read.
   int writeASCIIdata(std::ostream& os) noexcept(false);
   ...
   /// @throw if any stream error occurs.
   int writeBinaryFile(std::string filename) noexcept(false);
   ...
   int readBinaryFile(std::string filename) noexcept(false);
   ...
   ///        -4 header has not yet been read.
   /// @throw if a gap in time is found between consecutive records.
   int initializeWithBinaryFile(std::string filename) noexcept(false);
   ...
   gpstk::CommonTime startTime(void) const noexcept(false)
   { return JulianDate(startJD); }

   /// Return the end time of the data
   gpstk::CommonTime endTime(void) const noexcept(false)
...
private:
   ...
   void readBinary(char *ptr, size_t size) noexcept(false);
   ...
   void readBinaryHeader(std::string filename) noexcept(false);
   ...
   ///        -4 header has not yet been read.
   /// @throw if a gap in time is found between consecutive records.
   int readBinaryData(bool save) noexcept(false);
   ...
   ///        -2 EOF was reached
   ///        -3 input stream is not open or not valid
   int readBinaryRecord(std::vector<double>& data_vector) noexcept(false);
   ...
   /// -3 or -4 => initializeWithBinaryFile() has not been called, or reading failed.
   int seekToJD(double JD) noexcept(false);
