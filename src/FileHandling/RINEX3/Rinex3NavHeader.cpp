...
namespace gpstk
{
   ...
   fromString(const std::string str)
   {
      ...
      if (STR == std::string("GAL"))
         ...
         type = GPSB;
	  else if (STR == std::string("BDSA"))
		  type = BDSA;
	  else if (STR == std::string("BDSB"))
		  type = BDSB;
	  else if (STR == std::string("QZSA"))
		  type = QZSA;
	  else if (STR == std::string("QZSB"))
		  type = QZSB;
	  else if (STR == std::string("IRNA"))
		  type = IRNA;
	  else if (STR == std::string("IRNB"))
		  type = IRNB;
      else
      {
      ...
      while (!(valid & validEoH)) 
      {
         ...
         else if(thisLabel == stringTimeSysCorr)
         {
            ...
            if(tc.type == TimeSystemCorrection::GLGP ||
               ...
               tc.type == TimeSystemCorrection::IRGL || 
               tc.type == TimeSystemCorrection::IRGP || 
               tc.type == TimeSystemCorrection::IRUT)
   ...
   void Rinex3NavHeader::reallyPutRecord(FFStream& ffs) const
   {
      ...
      if(valid & validTimeSysCorr)
      {
         ...
         for(it=mapTimeCorr.begin(); it != mapTimeCorr.end(); ++it)
         {
            ...
            {
               strm << tc.asString4() << " "
                   << doubleToScientific(tc.A0, 17,10,2)
                   << doubleToScientific(tc.A1, 16,9,2)
            ...
            else
            {
               ...
               {
                     // "DELTA-UTC: A0,A1,T,W" R2.11 GPS
                  strm << "   " << doubleToScientific(tc.A0,19,12,2) 
                      << doubleToScientific(tc.A1,19,12,2)
               ...
               else if(tc.asString4() == "GLGP")
               {
                  ...
                  strm << right << setw(6) << tc.refYr
                       ...
                       << doubleToScientific(tc.A0,19,12,2) << setw(23) << ' '
                       << stringCorrSysTime;
               }
               else if(tc.asString4() == "SBUT")
               {
                     // "D-UTC A0,A1,T,W,S,U" R2.11 GEO
                  strm << doubleToScientific(tc.A0,19,12,2) << doubleToScientific(tc.A1,19,12,2)
                       << right << setw(7) << tc.refSOW
                       << right << setw(5) << tc.refWeek
...
//         lineMap[stringIonBeta] = lici->second == rici->second;
   ...
   std::ostringstream& operator<< (std::ostringstream& s, const IonoCorr::CorrType ty)
   {
	   if (ty == IonoCorr::CorrType::Unknown) s << "Unknown";
	   else if (ty == IonoCorr::CorrType::GAL)  s << "GAL";
	   else if (ty == IonoCorr::CorrType::GPSA)  s << "GPSA";
	   else if (ty == IonoCorr::CorrType::GPSB)  s << "GPSB";
	   else if (ty == IonoCorr::CorrType::BDSA)  s << "BDSA";
	   else if (ty == IonoCorr::CorrType::BDSB)  s << "BDSB";
	   else if (ty == IonoCorr::CorrType::QZSA)  s << "QZSA";
	   else if (ty == IonoCorr::CorrType::QZSB)  s << "QZSB";
	   else if (ty == IonoCorr::CorrType::IRNA)  s << "IRNA";
	   else if (ty == IonoCorr::CorrType::IRNB)  s << "IRNB";
	   else  s << "Unknown";
	   return s;
   }
