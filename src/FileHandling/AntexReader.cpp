   ...
   Antenna AntexReader::fillAntennaData( const std::string& firstLine )
   {
      ...
      while( label != endOfAntenna )
      {
         ...
         if( label == startOfFreq )
         {
            ...
            else if( freqString == "C05" ) freq = Antenna::C05;
...
//       formattedGetLine(line, true);
      ...
      noexcept(false)
   {
      ...
      antexStream.close();
      antexStream.open( fileName.c_str(), std::ios::in );
...
//                formattedGetLine(line, true);
   ...
   void AntexReader::open(const char* fn)
   {
      ...
      try
      {
         antexStream.open(fn, std::ios::in);
      }
      catch(...)
      {
	     cerr << "open antex file error!" << endl;
	     exit(-1);
      }
   ...
   void AntexReader::open(const string& fn)
   {
      ...
      try
      {
         antexStream.open(fn.c_str(), std::ios::in);
      }
      catch(...)
      {
	     cerr << "open antex file error!" << endl;
