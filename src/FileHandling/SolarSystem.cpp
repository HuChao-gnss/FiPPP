...
namespace gpstk {
//------------------------------------------------------------------------------------
void SolarSystem::readASCIIheader(string filename) noexcept(false)
{
try {
   // open the file
   ifstream strm;
...
//------------------------------------------------------------------------------------
int SolarSystem::readASCIIdata(vector<string>& filenames) noexcept(false)
{
try {
   size_t i;
...
//------------------------------------------------------------------------------------
int SolarSystem::readASCIIdata(string filename) noexcept(false)
{
try {
...
//------------------------------------------------------------------------------------
int SolarSystem::writeASCIIheader(ostream& os) noexcept(false)
{
try {
...
//------------------------------------------------------------------------------------
int SolarSystem::writeASCIIdata(ostream& os) noexcept(false)
{
try {
   if(EphemerisNumber < 0) return -4;
...
//------------------------------------------------------------------------------------
int SolarSystem::writeBinaryFile(string filename) noexcept(false)
{
try {
   int recLength;
   size_t i;
...
//------------------------------------------------------------------------------------
int SolarSystem::readBinaryFile(string filename) noexcept(false)
{
try {
   int iret;
...
//------------------------------------------------------------------------------------
int SolarSystem::initializeWithBinaryFile(string filename) noexcept(false)
{
try {
   int iret;
...
int SolarSystem::computeState(double tt,
                              ...
                              double PV[6],
                              bool kilometers) noexcept(false)
{
try {
   int iret,i;
...
Position SolarSystem::WGS84Position(Planet body, const CommonTime time,
   const EarthOrientation& eo) noexcept(false)
{
try {
   double PV[6];
...
//------------------------------------------------------------------------------------
// private
void SolarSystem::readBinary(char *ptr, size_t size) noexcept(false)
{
try {
   istrm.read(ptr,size);
...
//------------------------------------------------------------------------------------
// private
void SolarSystem::readBinaryHeader(std::string filename) noexcept(false)
{
try {
   int recLength;
   size_t i;
...
int SolarSystem::readBinaryData(bool save) noexcept(false)
{
try {
...
// -4 EphemerisNumber is not defined
// For -3,-4 : initializeWithBinaryFile() has not been called, or reading failed.
int SolarSystem::readBinaryRecord(vector<double>& data_vector) noexcept(false)
{
try {
   if(!istrm) return -3;
...
// -4 EphemerisNumber is not defined
// For -3,-4 : initializeWithBinaryFile() has not been called, or reading failed.
int SolarSystem::seekToJD(double JD) noexcept(false)
{
try {
   if(!istrm) return -3;
   if(istrm.eof() || !istrm.good()) return -3;
