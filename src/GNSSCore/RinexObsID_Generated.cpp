...
namespace gpstk
{
         ...
         : rinexVersion(version)
   {
      ...
      const auto& tc = char2tc.find(modStrID[3]);//TrackingCode,Channel
      const auto& cb = char2cb.find(modStrID[2]);//CarrierBand,Frequency
      const auto& ot = char2ot.find(modStrID[1]);//observation type
      if (tc != char2tc.end())
         code = tc->second;
      if (cb != char2cb.end())
         band = cb->second;
