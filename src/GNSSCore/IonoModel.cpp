...
namespace gpstk
{
       ...
       noexcept(false)
   {
        ...
        */
        ...
        // see ICD-GPS-200 20.3.3.3.3.2
        // Klobuchar model is modelled for L1-frequency code ionospheric delays, 
        // with unit of seconds.
       t_iono = t_iono * std::pow(L1_FREQ_GPS, 2.0) / std::pow(freq, 2.0);  //  GAMMA_GPS = (fL1 / fL2)^2

       // for test  
       if (debug)
       {
           cout << "IonoModel f1^2/fi^2:"
               << " freq:" << freq
               << std::pow(L1_FREQ_GPS, 2.0) / std::pow(freq, 2.0) << endl;
       }
